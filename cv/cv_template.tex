...
\begin{document}
	...
	\section*{Current Positions \& Affiliations}
	...
	\begin{singledatelist}
		...
		\dateitem[{{ edu.year }}]{{- edu.degree | l_bracket }}, {% if edu.subject %}{{ edu.subject }}, {% endif %}{{ edu.university }}.}
		{%- endfor %}
	\end{singledatelist}
	...
	\begin{infolist}
		...
		{%- else -%}
		\ \textit{{ paper.journal | lr_bracket }}, {{ paper.volume }}({{ paper.number }}), {{ paper.page_start }}--{{ paper.page_end }}.}
		{%- endif %}
		{%- endif %}
		{%- endfor %}
	\end{infolist}
	...
	\begin{infolist}
		...
		{%- if paper.authors|length > 1 -%}
			...
			{%- set author_count = [] -%}
			{%- for author in paper.authors -%}
				{%- if author != "Fredrik Sävje" -%}
					{% set __ = author_count.append(1) %}
					{%- if author_count|length == 1 -%}
						\ {{ author }}
					{%- elif author_count|length == loop.length - 1 -%}
						\ and {{ author }}
					{%- else -%}
						, {{ author }}
					{%- endif -%}
